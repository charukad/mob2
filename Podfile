...
# Custom fix for react-native-maps / Google Maps
# Set the path to the react-native-maps directory
$RNMapPath = '../node_modules/react-native-maps'

target 'SriLankaTourismGuide' do
  use_expo_modules!
...
# @generated end react-native-maps
  ...
  # Add this to the end to make sure it links correctly
  pod 'GoogleMaps'
  ...
  post_install do |installer|
    ...
    installer.pods_project.targets.each do |target|
      target.build_configurations.each do |config|
        # Fix for Google Maps with Bridgeless mode
        if target.name == 'Google-Maps-iOS-Utils'
          config.build_settings['CLANG_ENABLE_MODULES'] = 'NO'
        end
        
        # Fix for React Native 0.71+
        if target.name == "react-native-google-maps"
          config.build_settings['CLANG_ENABLE_MODULES'] = 'NO'
        end

        # Additional settings for Google Maps
        if target.name == 'Google-Maps-iOS-Utils' || target.name == 'GoogleMaps' || target.name == 'react-native-google-maps'
